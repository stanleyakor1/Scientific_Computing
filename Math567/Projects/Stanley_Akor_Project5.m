...
function [U,E,Error,x]=Upwind(dx,h)
    ...
    if any(mu>1) || isempty(mu) || length(x)<2
        
        disp('It is not possible to solve the upwind scheme with the chosen step sizes.');

        % If the CFL criterion isn't met, display the above and print an
        % error message
    ...
    %%% Solving for everywhere else apart from the initial and boundary
    %%% conditions
